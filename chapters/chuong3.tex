...
Dữ liệu nghiên cứu được thu thập từ nguồn dữ liệu quốc gia thuộc lĩnh vực bất động sản Singapore trong giai đoạn 2012--2025.
...
\hline
Nguồn dữ liệu quốc gia thuộc lĩnh vực bất động sản Singapore \\
\hline
...
\centering
\fontsize{13pt}{14.95pt}\selectfont
\caption{Ngưỡng phân loại tối ưu theo từng mô hình (V10)}
\label{tab:threshold}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}X|c|c|c|c|}
\hline
\textbf{Mô hình} & \textbf{Ngưỡng} & \textbf{F1-test} & \textbf{ROC-AUC} & \textbf{PR-AUC} \\
\hline
A7\_XGBoost-CLEAN & 0.8318 & 0.881 & 0.995 & 0.932 \\
\hline
A11\_BiLSTM-Cls & 0.9816 & 0.764 & 0.991 & 0.857 \\
\hline
A2\_DAE-Mahalanobis & 0.0779 & 0.391 & 0.953 & 0.356 \\
\hline
A10\_AttnBiLSTM-AE & 0.0010 & 0.423 & 0.953 & 0.418 \\
\hline
\textbf{A12\_Ensemble} & \textbf{0.9743} & \textbf{0.848} & \textbf{0.996} & \textbf{0.872} \\
\hline
\end{tabularx}
\end{table}
...
\begin{itemize}
  \item \textbf{Mysql}: MySQL 8.0, volume persist.
  \item \textbf{Redis}: Redis 7, broker cho Celery.
  \item \textbf{Minio}: MinIO S3-compatible storage.
  \item \textbf{Celery-worker}: Python worker (4 concurrent tasks).
  \item \textbf{Backend}: FastAPI + Uvicorn.
  \item \textbf{Frontend}: Next.js production build.
\end{itemize}`'
...
\caption{So sánh kết quả phát hiện bất thường trên tập kiểm tra (V10)}
\label{tab:ket_qua_ad}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}X|c|c|c|c|c|}
\hline
\textbf{Phương pháp} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} & \textbf{ROC-AUC} & \textbf{PR-AUC} \\
\hline
Isolation Forest\textsuperscript{*} & --- & --- & --- & --- & --- \\
\hline
Local Outlier Factor\textsuperscript{*} & --- & --- & --- & --- & --- \\
\hline
OC-SVM\textsuperscript{*} & --- & --- & --- & --- & --- \\
\hline
Rule-based (13 luật)\textsuperscript{*} & --- & --- & --- & --- & --- \\
\hline
A2\_DAE-Mahalanobis & 0.306 & 0.541 & 0.391 & 0.953 & 0.356 \\
\hline
A10\_AttnBiLSTM-AE & 0.569 & 0.337 & 0.423 & 0.953 & 0.418 \\
\hline
A11\_BiLSTM-Cls & 0.850 & 0.694 & \textbf{0.764} & 0.991 & 0.857 \\
\hline
A7\_XGBoost-CLEAN & 0.895 & 0.867 & \textbf{0.881} & 0.995 & 0.932 \\
\hline
\textbf{A12\_Ensemble (đề xuất)} & \textbf{0.795} & \textbf{0.908} & \textbf{0.848} & \textbf{0.996} & \textbf{0.872} \\
\hline
\end{tabularx}
\end{table}

\textit{Ghi chú:} \textsuperscript{*}Các baseline Isolation Forest, LOF, OC-SVM và Rule-based chưa được đánh giá định lượng trong phạm vi thực nghiệm hiện tại vì project tập trung vào supervised/semi-supervised. Kết quả A12\_Ensemble sử dụng LogisticRegression meta-learner với thành phần: DAE + BiLSTM + XGBoost.

\textbf{Phân tích kết quả:} Mô hình A7\_XGBoost-CLEAN đạt F1 = 0.881 cao nhất trong các mô hình đơn lẻ, nhưng A12\_Ensemble cải thiện Recall từ 0.867 lên \textbf{0.908} (tăng 4.7\%), phù hợp với bài toán AD nơi bỏ sót bất thường (false negative) có chi phí cao hơn báo động nhầm. ROC-AUC của ensemble đạt \textbf{0.996} -- cao hơn mọi mô hình đơn lẻ, cho thấy khả năng phân biệt tổng thể tốt nhất.
...
\centering
\fontsize{13pt}{14.95pt}\selectfont
\caption{Kết quả đánh giá báo cáo NLP trước và sau LoRA fine-tune (dự kiến)}
...
\hline
Qwen2-1.5B (zero-shot) & --- & --- & --- \\
\hline
\textbf{Qwen2-1.5B + LoRA} & --- & --- & --- \\
\hline
Gemma-2B (zero-shot) & --- & --- & --- \\
\hline
\textbf{Gemma-2B + LoRA} & --- & --- & --- \\
...
\textit{Ghi chú:} Đánh giá BLEU/ROUGE và human evaluation chưa được thực hiện; sẽ bổ sung khi hoàn thành giai đoạn fine-tune và thu thập reference report từ chuyên gia nghiệp vụ. Mô hình Qwen2-1.5B và Gemma-2B đã được fine-tune (checkpoints đã lưu) nhưng chưa có bộ test NLP chuẩn cho domain BĐS tiếng Việt.
...
\centering
\fontsize{13pt}{14.95pt}\selectfont
\caption{Latency end-to-end theo kích thước CSV đầu vào (dự kiến)}
\label{tab:latency}
\begin{tabularx}{\textwidth}{|c|c|c|c|}
\hline
\textbf{Kích thước CSV} & \textbf{Preprocess} & \textbf{Detect} & \textbf{Total (upload$\to$PDF)} \\
\hline
1.000 dòng & $\leq$ 20s & $\leq$ 10s & $\leq$ 60s \\
\hline
5.000 dòng & $\leq$ 60s & $\leq$ 30s & $\leq$ 3 phút \\
\hline
10.000 dòng & $\leq$ 120s & $\leq$ 60s & $\leq$ 5 phút \\
\hline
\end{tabularx}
\end{table}

\textit{Ghi chú:} Số liệu latency là ước tính dựa trên benchmark thành phần riêng lẻ; cần đo end-to-end trên môi trường production thực tế. Report generation (LLM inference + PDF export) chiếm phần lớn thời gian xử lý.
